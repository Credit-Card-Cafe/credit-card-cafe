...
{#if bank.modifiers}
    {#each Object.entries(bank.modifiers) as [modifierId, modifierName]}
        ...
        {:else}
        <button on:click={() => addModifierToUser(modifierId)} class="m-2 p-2 bg-black/[0.1] dark:bg-white/[0.1] dark:text-white-warm hover:bg-black/[0.4] dark:hover:bg-white/[0.4] rounded-md inline-flex flex-row items-center">{modifierName}</button>
        {/if}
    {/each}
{/if}