...
<button class="md:shadow-lg p-1 md:p-2 rounded-lg mb-1 md:mb-4 mx-auto md:mx-1 flex flex-col md:w-auto w-11/12" on:click={() => toggleContext()}>
    ...
    <span class={`my-2 flex flex-row justify-start flex-wrap md:${context ? "inline" : "hidden"}`}>
        {#each categories[category] as reward}
            ...
            reward.custom ? "dark:bg-white/[0.1] bg-black/[0.1] hovertip outline outline-theme-green m-2 p-2 rounded-md inline-flex flex-col items-center relative" 
                          : "bg-black/[0.1] dark:bg-white/[0.1] hovertip m-2 p-2 rounded-md inline-flex flex-col items-center relative"}>
