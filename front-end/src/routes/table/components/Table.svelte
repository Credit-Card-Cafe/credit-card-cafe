<script lang="ts">
    ...
    import {type CardObject, COP, TLS } from "$lib/types";
    ...
    import { getOneBankByName } from "../../api/banks/banks";

    export let tablelist:CardObject[];
    export let walletIDList:string[];
    export let trackingIDList:string[];
    export let tableCall: 0 | 1 | 2; //0 = normal table, 1 = table list, 2 = empty table
    ...
    let availableQueries: COP[] = Object.values(COP);
...
<main class="bg-theme-white dark:text-theme-text-white dark:bg-theme-black flex flex-col px-4">
    {#if walletIDList.length <= 0 && trackingIDList.length <= 0 && tableCall == 2}
    <section class="flex flex-col gap-y-8 lg:ml-16 lg:w-3/4 mb-12">
        ...
        <h2 class="text-2xl lg:text-4xl font-semibold">Use our table to compare any property of cards in your wallet, and cards you are interested in!</h2>
        <a href="/s/get-started/" class="hidden lg:block mt-6 bg-theme-blue hover:bg-theme-blue-hov py-4 px-20 w-full lg:w-fit justify-center text-white inline-flex flex-row rounded-lg text-xl font-semibold items-center">Try it out now!</a>
    </section>
    {/if}
    ...
    <div class="hidden lg:block text-center text-sm">Track cards or add cards to your wallet to view them on the table!</div>
    {/if}
    ...
    <table class="dark:text-white-warm max-w-fit rounded-r-xl table-fixed overflow-x-scroll">
        ...
        {#each tablelist as card}
            ...
            <tr class="even:bg-theme-green/25 dark:even:bg-inherit dark:border-t-2 dark:border-theme-green/10">
                <td class="table-cell flex-col items-center text-left odd:rounded-xl odd:rounded-r-none odd:rounded-l-xl">
                ...
                {#await getOneBankByName(card.card_bank)}
                    <td class="even:bg-black/5 odd:last:rounded-r-xl ">{card.card_bank}</td>
                {:then bank} 
                    <td class="even:bg-black/5 odd:last:rounded-r-xl "><a href="/bank/{bank?.bank_url}">{bank ? bank.bank_name : card.card_bank}</a></td>
                {/await}
                <TableData class="even:bg-black/5 odd:last:rounded-r-xl " queries={selectedQueries} card={card}></TableData>
            </tr>
            {/if}
        {/each}
        <!---------------------->
