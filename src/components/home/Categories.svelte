...
{#each categories as category}
    <div class="pt-20 md:pt-0 bg-gradient-to-t from-green-100 dark:from-green-700 to-alt dark:to-main-gray to-80% md:min-h-[calc(100vh-4rem)] flex flex-col justify-evenly md:snap-end">
      <div class="flex flex-col md:flex-row justify-between mx-16">
        ...
        <button on:click={() => goto(`/table/${category.id}`)} class="block px-4 py-2 md:px-8 md:py-4 my-4 md:my-0 text-lg md:text-2xl text-center shadow rounded-md bg-black/[0.1] dark:bg-white/[0.05] md:mx-0 mx-auto hover:bg-black/[0.3] transition-all flex flex-row items-center">View Cards In Table
