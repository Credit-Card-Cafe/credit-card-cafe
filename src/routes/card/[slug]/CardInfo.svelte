<script lang="ts">
    import { type CreditCardType, CardConsumer, CardInfoLocation } from "$lib/types";
    export let card:CreditCardType;
    import { lists, dataField } from "$lib/fields";
    
</script>

<div id="cardInfo" class="dark:text-white-warm">
    <div>{card.name}</div>
    <div><a href="/bank/{card.bank_id}">{card.bank}</a></div>
    <div>{card.brand}</div>
    <div>{card.network}</div>
    {#if card.rewards}
    <div id="rewards" class="mb">
        <div class="large">Rewards:</div>
        <ul>
        {#each Object.keys(card.rewards) as reward}
            {#if card.redemption}
                {#if card.redemption == "Cash Back"}
                    <li>{card.rewards[reward]}% {card.redemption} on {lists.rewardCategories[reward]} purchases</li>
                {:else}
                    <li>{card.rewards[reward]}x {card.redemption} on {lists.rewardCategories[reward]} purchases</li>
                {/if}
            {:else}
            <li>{card.rewards[reward]}x {lists.rewardCategories[reward]}</li>
            {/if}
        {/each}
        </ul>
    </div>
    {/if}
    
    {#if card.fees}
    <div id="fees" class="mb">
        <div class="large">{dataField["fees"].name}</div>
        <table>
            {#each Object.keys(dataField["fees"]["data"]) as field}
                {#if field in card["fees"]}
                    <tr>
                        <td>{dataField["fees"]["data"][field].name}</td>
                        <td>{#if card["fees"][field] == 0}No {dataField["fees"]["data"][field].name}
                            {:else if dataField["fees"]["data"][field].unit == "$"}
                                ${card["fees"][field]}
                            {:else}
                                {card["fees"][field]}{dataField["fees"]["data"][field].unit}
                            {/if}
                        </td>
                    </tr>
                {/if}
            {/each}
        </table>
        <div class="small">*Some Fees May Not Be Listed</div>
    </div>
    {/if}

    {#if card.physical}
    <div id="physical" class="mb">
        <div class="large">Physical Properties:</div>
        <ul>
        {#if card.physical.material}<li>Material: {card.physical.material}</li>{/if}
        {#if card.physical.embossed == "Yes"}<li>Embossed Name</li>{/if}
        {#if card.physical.tap_to_pay == "Yes"}<li>Tap-to-pay enabled</li>{/if}
        {#if card.physical.chip == "Yes"}<li>Features a Chip</li>{/if}
        {#if card.physical.info_location}
        <li>
            {#if card.physical.info_location == CardInfoLocation.Front}
                Information Displayed on the Front
            {:else if card.physical.info_location == CardInfoLocation.Back}
                Information Displayed on the Back
            {:else if card.physical.info_location == CardInfoLocation.Both}
                Information Displayed on Back and Front
            {/if}
        </li>
        {/if}
        </ul>
    </div>
    {/if}

    {#if card.link}
        <div><a href={card.link}>Apply to the card here</a></div>
    {/if}
</div>

<style>
    tr,td {
        padding: 1rem;
    }
    table {
        border: 1px solid black;
        border-radius: 5px;
    }
    table > tr:nth-child(even) > td {
        border-top: 1px solid black;
    }
    tr > td:nth-child(2) {
        border-left: 1px solid black;
    }
    .large {
        font-size: 2rem;
    }
    .small {
        font-size: 0.75rem;
    }
    .mb {
        margin-bottom: 2rem;
    }
    #cardInfo {
        margin-top: 1rem;
        padding: 1rem;
    }
    @media (min-width: 768px) {
        #cardInfo {
            margin-left: 20rem;
            margin-right: 20rem;
            margin-top: 5rem;
            display: flex;
            flex-direction: column;
        }
}
</style>