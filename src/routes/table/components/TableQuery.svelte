...
{#if object}
    ...
    <label for={`query${object + field}`} class={`label mr-1 ml-1 last:mr-0 ${isChecked ? "grn dark:text-gray-900 text-alt" : "bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700"}`}>{dataField[object]["data"][field].name}</label> 
{:else if isObject}
    <input id={`query${field}`} class="hidden" type="checkbox" value={field} on:change={() => addQuery()}>
    <label for={`query${field}`} class={`label ${isChecked ? "blu dark:text-gray-900 text-alt" : "bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 m-1"}`}>{dataField[field].name}</label>
    {#if isChecked} 
